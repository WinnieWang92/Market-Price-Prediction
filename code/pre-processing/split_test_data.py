...
advss = ['1458', '2259', '2261', '2821', '2997', '3358', '3386', '3427', '3476','all']
# advss = ['all']
data_folder = '../data/'
for aadv in advss:
    print 'running ' + aadv
    test_yzx_bid = data_folder + aadv + '/test.yzx.bid.txt'
    test_yzx_win = data_folder + aadv + '/test.yzx.win.txt'
    test_yzx_lose = data_folder + aadv + '/test.yzx.lose.txt'


    original_ecpc = 0.  # original eCPC from train.yzx.base.txt
    total_cost_train = 0  # total original cost during the train data

    # read in test.yzx.base for original_ecpc
    fi = open(test_yzx_bid, 'r')
    first = True
    imp_num = 0
    clk_num = 0
    for line in fi:
    ...
    print 'Finish loading test.yzx.bid.txt'
    ...
    adv_bid_discount = {'1458': 4, '2259': 4, '2261': 4, '2821': 4, '2997': 4, '3358': 4, '3386': 4, '3427': 4,
                        '3476': 4, 'all': 4}
    discount = 4
    for adv in adv_bid_discount:
        if adv in test_yzx_bid:
    ...
    fo1 = open(test_yzx_win, 'w')
    # fo2 = open(train_wzb_imp, 'w')
    fo2 = open(test_yzx_lose, 'w')
    print 'Begin to bid'
    for line in fi:
        data = ints(line.replace(":1", "").split())
        pred = 0.0
